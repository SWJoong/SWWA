...
> 상태: **계획 단계 (T-00 완료)**. 구현은 [docs/plan/06-harness-engineering.md](docs/plan/06-harness-engineering.md)의 WBS 순서로 진행한다.
...
| MCP 도구 | `check_html` · `check_contrast` · `lookup_checkpoint` · `get_checklist` · `audit_url` · `browser_status` · `estimate_cert_readiness` |
| MCP 프롬프트·리소스 | `review-markup` · `audit-report` / `swwa://kwcag22` 등 6종 |

- 정적 검사(jsdom + axe-core 한국어 로케일 + 자체 `k-` 규칙)는 브라우저 없이 동작합니다.
- 브라우저 감사(playwright-core + @axe-core/playwright + 자체 `b-` 규칙)는 설치된 Chrome/Edge를 자동 사용하며, 없으면 `npx playwright install chromium`을 안내합니다.
...
KWCAG 2.2 원문 저작권은 국립전파연구원(방송통신표준)에 있으며, 이 프로젝트는 검사항목 명칭·요구 문장만 인용하고 설명은 자체 요약합니다. axe-core(MPL-2.0)를 검사 엔진으로 사용합니다. 자세한 출처는 `docs/plan/07-references.md`와 `assets/sources.json`(예정)을 참조하세요.
